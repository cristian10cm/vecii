...
const UserInfo = () => {
  const { setInformation,setName } = setHousing();
  ...
  const information = setHousing().information?.location
  ...
  const comprobarDatos = (
    ...
    if (
      cel == useData?.cellPhoneNumber &&
      phone == useData.phoneNumber &&
      email == useData.email &&
      lastName == useData.lastName &&
      name == useData.firstName
    ) {
  ...
  const peticion = async (idUser: string) => {
    try {
      const api = await http.get(`users/${idUser}`, {
        headers: {
  ...
  const enviar = async () => {
    ...
    try {
      const body = {
        ...
        roles: ['17c64b38-d01a-4b2a-a5dc-cd5f496e5114'],
        phoneNumber: phoneNumber.current?.value,
        cellPhoneNumber: cellPhoneNumber.current?.value,
      };
      const update = await axios.patch(
        `https://api.vecii.com.co/api/v1/users/${id}`,
        body
      );
      if (update) {
      setUseData(update.data)

        setReset((x) => x + 1);
      setName(firstName?.current?.value || '',lastName?.current?.value || '')
      }
  ...
  return (
    <div className='container_infoUser'>
      <div className='container_infoUser_img'>
        <img
      ...
      <h2 className='container_infoUser_title'>{` ${information?.unit.name} ${information?.housing.name}`}</h2>
      {useData ? (
